...
object VectorSearchEngine {
    ...
    suspend fun loadFromFile(context: Context) {
        withContext(Dispatchers.IO) {
    ...
    suspend fun exportVectorsJsonToDownloads(context: Context): String {
        return withContext(Dispatchers.IO) {
            val snapshot = getAllEntries()
            if (snapshot.isEmpty()) {
                return@withContext "No vector data to export."
            }
            val jsonString = try {
                Json.encodeToString(snapshot)
            } catch (e: Exception) {
                Log.e("VectorSearchEngine", "Error serializing vectors to JSON: ${e.message}", e)
                return@withContext "Error: Could not serialize data to JSON. ${e.localizedMessage}"
            }
            ...
            val uri = try {
                ...
                return@withContext "Error: Could not create file entry in MediaStore. ${e.localizedMessage}"
            }


            if (uri == null) {
                Log.e("VectorSearchEngine", "MediaStore URI is null, cannot save file.")
                return@withContext "Error: Could not get URI for saving the file."
            }
